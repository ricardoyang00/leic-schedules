...
private:
    ...
    Student* searchByCode(Node* root, const int& studentCode); // Recursive method to search for a student with its StudentCode
    void inOrderTraversal(Node* node, function<void(const Student&)> action);
public:
    StudentBST();
    Node* getRoot();
    void setRoot(Node* newRoot);
    void insertStudent(const int& studentCode, const string& studentName, vector<Class> ucToClasses); // Insert a new student into the BST
    Student* searchByCode(const int& studentCode); // Search for a student with its StudentCode
